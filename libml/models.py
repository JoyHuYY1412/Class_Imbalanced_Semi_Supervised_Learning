...
class Res18(ClassifySemi):
    ...
    def classifier(self, x, scales, filters, training, getter=None, **kwargs):
        ...
        def conv_args(k, f, s=1):
            return dict(kernel_size=k, filters=f, strides=s, padding='same',
        ...
        def residual(x0, filters, stride=1, activate_before_residual=False):
            ...
            x = tf.layers.conv2d(x, **conv_args(3, filters, stride))
            x = leaky_relu(tf.layers.batch_normalization(x, **bn_args))
            x = tf.layers.conv2d(x, **conv_args(3, filters))

            if x0.get_shape()[3] != filters:
                x0 = tf.layers.conv2d(x0, **conv_args(1, filters, stride))
            return x0 + x

        with tf.variable_scope('classify', reuse=tf.AUTO_REUSE, custom_getter=getter):
            # resnet 18: filters = 64
            y = tf.layers.conv2d((x - self.dataset.mean) / self.dataset.std, **conv_args(7, filters, 2))
            y = leaky_relu(tf.layers.batch_normalization(y, **bn_args))
            y = tf.layers.max_pooling2d(y, [2,2], 2)

            # layer1
            for i in range(2):
                y = residual(y, filters)   

            # layer2
            y = residual(y, filters*2, stride=2)   
            y = residual(y, filters*2, stride=1)   

            # layer3
            y = residual(y, filters*4, stride=2)   
            y = residual(y, filters*4, stride=1)   

            # layer4
            y = residual(y, filters*8, stride=2)   
            y = residual(y, filters*8, stride=1)  
            y = leaky_relu(tf.layers.batch_normalization(y, **bn_args)) 
            y = embeds = tf.reduce_mean(y, [1, 2])  #avgpool
...
class MultiModel(CNN13, Res18, ResNet, ShakeNet):
    MODELS = ('cnn13', 'res18', 'resnet', 'shake')
    MODEL_CNN13, MODEL_RES18, MODEL_RESNET, MODEL_SHAKE = MODELS
    ...
    def classifier(self, x, arch, **kwargs):
        ...
        elif arch == self.MODEL_RES18:
            return Res18.classifier(self, x, **kwargs)
