...
def _load_stl10():
    ...
    with tempfile.NamedTemporaryFile() as f:
        ...
        unlabeled_set = {'images': _imgs, 'labels': np.zeros(100000, dtype=np.uint8)}
    ...
    with tempfile.NamedTemporaryFile() as f:
        ...
        for batch in range(1, 6):
            data_dict = scipy.io.loadmat(
                tar.extractfile('cifar-10-batches-mat/data_batch_{}.mat'.format(batch)))
        ...
        test_set = {'images': data_dict['data'], 'labels': data_dict['labels'].flatten()}
    train_set['images'] = _encode_png(unflatten(train_set['images']))
    test_set['images'] = _encode_png(unflatten(test_set['images']))
    return dict(train=train_set, test=test_set)


def _load_cifar10_LT_20():
    def unflatten(images):
        return np.transpose(images.reshape((images.shape[0], 3, 32, 32)), [0, 2, 3, 1])
...
def _load_cifar100():
    ...
    with tempfile.NamedTemporaryFile() as f:
        ...
        train_set = {'images': data_dict['data'], 'labels': data_dict['fine_labels'].flatten()}
        data_dict = scipy.io.loadmat(tar.extractfile('cifar-100-matlab/test.mat'))
        test_set = {'images': data_dict['data'], 'labels': data_dict['fine_labels'].flatten()}
    train_set['images'] = _encode_png(unflatten(train_set['images']))
    test_set['images'] = _encode_png(unflatten(test_set['images']))
    return dict(train=train_set, test=test_set)


def _load_cifar100_rand():
    def unflatten(images):
        return np.transpose(images.reshape((images.shape[0], 3, 32, 32)), [0, 2, 3, 1])
...
def _load_fashionmnist():
    ...
    for split, split_file in split_files:
        with tempfile.NamedTemporaryFile() as f:
            request.urlretrieve(URLS['fashion_mnist'].format(image_filename.format(split_file)),
                                f.name)
        ...
        with tempfile.NamedTemporaryFile() as f:
            request.urlretrieve(URLS['fashion_mnist'].format(label_filename.format(split_file)),
                                f.name)
            with gzip.GzipFile(fileobj=f, mode='r') as data:
...
CONFIGS = dict(
    ...
    cifar10_LT_20=dict(loader=_load_cifar10_LT_20, checksums=dict(train=None, test=None)),
    cifar10_LT_50=dict(loader=_load_cifar10_LT_50, checksums=dict(train=None, test=None)),
    cifar10_LT_50_RS=dict(loader=_load_cifar10_LT_50_RS, checksums=dict(train=None, test=None)),
    cifar10_LT_100=dict(loader=_load_cifar10_LT_100, checksums=dict(train=None, test=None)),
    cifar10_LT_100_RS=dict(loader=_load_cifar10_LT_100, checksums=dict(train=None, test=None)),
    cifar100=dict(loader=_load_cifar100, checksums=dict(train=None, test=None)),
    cifar100_LT_50=dict(loader=_load_cifar100_LT_50, checksums=dict(train=None, test=None)),
    cifar100_LT_100=dict(loader=_load_cifar100_LT_100, checksums=dict(train=None, test=None)),
    cifar100_LT_200=dict(loader=_load_cifar100_LT_200, checksums=dict(train=None, test=None)),
    cifar100_rand=dict(loader=_load_cifar100_rand, checksums=dict(train=None, test=None)),
    stl10=dict(loader=_load_stl10, checksums=dict(train=None, test=None)),
    stl10_LT_100=dict(loader=_load_stl10_LT_100, checksums=dict(train=None, test=None)),
    stl10_LT_50=dict(loader=_load_stl10_LT_50, checksums=dict(train=None, test=None)),
)
