...
def main(argv):
    ...
    if 'LT' in argv[0]:
        ...
        print('train_stats', train_stats)
        np.random.seed(FLAGS.seed)
        p_rand = np.array([np.random.random() for i in range(nclass)])
        p_rand = p_rand/max(p_rand)
        print('  Stats', ' '.join(['%.2f' % (100 * x) for x in p_rand]))
        train_stats = p_rand
    ...
    with tf.python_io.TFRecordWriter(target + '-label.tfrecord') as writer_label:
        ...
        for input_file in input_files:
            for record in tf.python_io.tf_record_iterator(input_file):
                if pos in label:
                    count_label = label.count(pos)
                    for count_label_i in range(count_label):
